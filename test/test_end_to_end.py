...
def test_parking_multiple_full(cars_config):
    ...
    assert (car_parking_status.count("Car parked") == 20) and (
        len(not_parked_cars) == 3
    )
...
def test_negative_parking_spot_no(cars_config):
    ...
    assert (
        response["status"] == "Car not parked"
        and response["error"] == "InvalidSpotNumberError"
    )
...
def test_greater_than_capacity_parking_spot_no(cars_config):
    ...
    assert (
        response["status"] == "Car not parked"
        and response["error"] == "ParkingCapacityExceededError"
    )
...
def test_already_parked_parking_spot_no(cars_config):
    ...
    assert (
        response["status"] == "Car not parked"
        and response["error"] == "SpotNotAvailableError"
    )
...
def test_parking_multiple_full_custom_sq_footage(cars_config):
    ...
    assert (car_parking_status.count("Car parked") == 52) and (
        len(not_parked_cars) == 48
    )
